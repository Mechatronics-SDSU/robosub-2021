...
"""The Python implementation of the GRPC helloworld.Freeter client."""
...
def run():
    ...
    with grpc.insecure_channel('unix:///sock/test.sock') as channel:
        stub = helloworld_pb2_grpc.FreeterStub(channel)
        response = stub.SayHello(helloworld_pb2.HelloRequest(name='Name'))
        print("Freeter client received: " + response.message)
